...
void http_conn::init()
{
    ...
    m_read_idx = 0;
    m_write_idx = 0;
    memset( m_read_buf, '\0', READ_BUFFER_SEZE );
...
bool http_conn::read()
{
    if ( m_read_idx >= READ_BUFFER_SEZE ) return false;

    int bytes_read = 0;
    while ( true )
    {
        bytes_read = recv( m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SEZE - m_read_idx, 0 );
        if ( bytes_read == -1 )
        {
...
http_conn::HTTP_CODE http_conn::parse_headers( char* text )
{
    ...
    else printf("oop! unkonw header %s\n", text);
